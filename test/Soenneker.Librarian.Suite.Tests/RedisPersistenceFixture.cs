...
internal sealed class RedisPersistenceFixture : IAsyncDisposable
{
    ...
    public RedisPersistenceFixture()
    {
    ...
    public RedisLibrarianDatabase CreateDatabase() => new(Key, Client, NullLogger.Instance);
    ...
    public async ValueTask DisposeAsync()
    {
        ...
        finally
        {
            try
            {
                ...
                string identity = string.Concat(System.Linq.Enumerable.Select(Key, c => ((int)c).ToString("X4")));
                string prefix = "librarian:{" + Convert.ToHexString(System.Security.Cryptography.SHA256.HashData(System.Text.Encoding.UTF8.GetBytes(identity))) + "}:batches:";
                foreach (EndPoint endpoint in connection.GetEndPoints())
                    await foreach (RedisKey redisKey in connection.GetServer(endpoint).KeysAsync(pattern: prefix + "*"))
